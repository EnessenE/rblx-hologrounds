...
BaseRay.Name = "Laser"
BaseRay.BrickColor = BrickColor.new("Black") --new yeller
...
BaseRayMesh.Scale = Vector3.new(0.4, 0.4, 0.4)
...
local function FireRay(StartPosition, TargetPosition, Hit)
	...
	local AX = (PX + Vector3.new(math.random(-0.21 * DX, (0.21 * DX),math.random(-0.21 * DX,math.min(script.Limit.Value, (0.21 * DX))), math.random(-0.21 * DX, (0.21 * DX)))))
	local AY = (PY + Vector3.new(math.random(-0.21 * DY, (0.21 * DY),math.random(-0.21 * DY,math.min(script.Limit.Value, (0.21 * DY))), math.random(-0.21 * DY, (0.21 * DY)))))
	local AZ = (PZ + Vector3.new(math.random(-0.21 * DZ, (0.21 * DZ),math.random(-0.21 * DZ,math.min(script.Limit.Value, (0.21 * DZ))), math.random(-0.21 * DZ, (0.21 * DZ)))))
	...
	for i, v in pairs(Rays) do
		...
		Ray.BrickColor = BrickColor.new("New Yeller") --new yeller
		Ray.Reflectance = 0.4 --.4
...
_G.l = function(StartPosition, TargetPosition)
	local RayHit = script.Hit
	FireRay(StartPosition, TargetPosition)
	if RayHit then
		PlaySound(TargetPosition, script.Hit)
	end
end
...
local function FireRay(StartPosition, TargetPosition, Hit)
	...
	for i, v in pairs(Rays) do
		local Ray = BaseRay:Clone()
		Ray.BrickColor = BrickColor.new("Black") --new yeller
		Ray.Reflectance = 0.4 --.4
		Ray.Transparency = 0 --.7
		local Mesh = Ray.Mesh
		Mesh.Scale = (Vector3.new(0.15, 0.15, (v.Distance / 1)) * 5)
...
_G.lightning = function(StartPosition, TargetPosition)
	local RayHit = script.Hit
	FireRay(StartPosition, TargetPosition)
	if RayHit then
		PlaySound(TargetPosition, script.Hit)
