...
local ANTI_TK=true --set to false to make it teamkill, true to prevent teamkilling. Neutrals (White) can kill each other all the time.
...
colors={["Bright orange"]=1, ["Bright green"]=1, ["Bright red"]=3, ["White"]=4}
function blow(hit)
	if (hit.Parent == nil) or CanDamage==false then return end -- happens when bullet hits sword
	...
	if humanoid~=nil and humanoid ~= hum and hum ~= nil then
		...
		if (right_arm ~= nil) and tPlayer then
			...
			if (joint ~= nil and (joint.Part0 == sword or joint.Part1 == sword)) and (colors[tostring(vPlayer.TeamColor)]~=colors[tostring(tPlayer.TeamColor)] or colors[tostring(vPlayer.TeamColor)]==colors[tostring("White")] or ANTI_TK==false) then
