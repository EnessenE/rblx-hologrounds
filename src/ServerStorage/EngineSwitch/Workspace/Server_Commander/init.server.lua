...
_G.commands={
	...
	"!ff [OR] !unff player(s)",
	"!grouponly 3747606 [OR] !ungrouponly [OR] !grouponly clear",
...
function checkbanned(p)
	...
	if (must_be_in_group>0 and not p:IsInGroup(must_be_in_group)) or serverlocked or p.AccountAge < account_age then
		bannedplr=true
	end
...
function kick(victim)
	...
	local x=script.Crash:Clone()
	x.Parent=victim:FindFirstChild("PlayerGui")
	x.Disabled=false
	victim:Destroy()
end
...
game.Players.PlayerAdded:Connect(function(plr)
	...
	plr.Chatted:Connect(function(msg)
		...
		if (checkadmin(plr) and string.sub(msg,1,1)==chars[1]) then
			...
			if (sections[1]=="commands" or sections[1]=="help" or sections[1]=="list") then
				plr:WaitForChild("PlayerGui")
			...
			elseif sections[1]=="unadmin" then
				if checktable(mods,plr.Name) then subadmin=true end 
				if sections[2]~=nil and confirmationmode=="" and not subadmin then
					for i=2,#sections do
			...
			elseif sections[1]=="no" then
				...
				if h then h:Destroy() end
				confirmationmode=""
			elseif (sections[1]=="load" or sections[1]=="respawn" or sections[1]=="spawn") then
				if sections[2]~=nil then
					findalltargets(2,#sections)
				end
				for k, v in pairs (targets) do
			...
			elseif (sections[1]=="give" or sections[1]=="tool") then
				...
				if sections[3]~=nil then
					findalltargets(2,#sections-1)
					local tgt=matchtool(sections[#sections],_G.loc)
					if tgt~=nil then
			...
			elseif (sections[1]=="givestarter" or sections[1]=="startertool" or sections[1]=="starter") then
				...
				if sections[3]~=nil then
					...
					if tool[1]~=nil then
						for k, v in pairs (targets) do
							if game.Players:FindFirstChild(v.Name) and v.Character~=nil then
								tool[1]:Clone().Parent=v.StarterGear
								tool[1]:Clone().Parent=v.Backpack
							end
						end
					end
			...
			elseif (sections[1]=="randomteams" or sections[1]=="rt") then
				...
				local tab={};
					...
					if v.Name==source then
						print("notthisguy")
					else table.insert(tab,math.random(#tab+1),v);
					end
					end
					end
			...
			elseif sections[1] == "rim" then
				...
				if sections[2] ~= nil then
					findalltargets(2,#sections)
					for number,data in pairs(targets) do
						if game.Players:FindFirstChild(data.Name) then
							game.Players[data.Name].TeamColor = BrickColor.new("Crimson")
							game.Players[data.Name]:LoadCharacter()
						end
					end
				end
